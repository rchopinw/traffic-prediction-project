...
if __name__ == '__main__':
    chrom = gen_chrom(pop_size=POP_SIZE, model='lstm')
    gen_min_acc = []
    for curnt_gen in range(MAX_GEN):
        chrom_acc = []
        for i, c in enumerate(chrom):
            lstm = LSTMr(params=c)
            x, x_t, y, y_t = lstm.get_xy(data=data_raw, norm=True)
            lstm.train(x=x, y=y)
            p = lstm.predict(x_t)
    ...
    optimal_model = LSTMr(params=optimal_args)
    ...
    # 保存预测数据和原始数据
    pd.DataFrame({'y_test': y_t.tolist(), 'y_predict': p.tolist()}).to_csv('predict_data_lstm.csv',
                                                                           encoding='utf_8_sig', index=False)

import itertools
for i in itertools.product('ABCD', repeat=4):
    print(i)
